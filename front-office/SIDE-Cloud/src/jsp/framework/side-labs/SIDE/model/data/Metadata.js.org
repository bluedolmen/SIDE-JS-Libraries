 ...
 * @static
 */
SIDE.model.data.Metadata.getClasses = function(nsp) {
	var result = null;
	SIDE.Util.get({
	  'url': '/alfresco/service/api/classes?nsp=Kinedoc1',
	  'cache': false,
	  'success': function(data) {
	  		result = data;
//	  		SIDE.pattern.Observer.publish('/metadata/classes',[{value: data}]);
	  },
	  'failure': function () {},
	  'async': false,
	  'dataType': 'json'
...
SIDE.model.data.Metadata.getClasses = function(nsp) {
	if (this.classes != null) {
		...
		var classes = SIDE.model.data.Metadata.getClasses('/alfresco/service/api/classes?nsp=' + nsp);
		...
		for(i = 0; i < classes.length; i++) {
			if (classes[i].isAspect === false) {
				//			names[j] = classes[i].name;
				if (classes[i].parent.name != 'dl:dataListItem') {
					// classes[i] is a content type
					contents[j] = classes[i];
					j++;
				} else {
					// classes[i] is a data list item
					dataListsItems[k] = classes[i];
					k++;
				}
			} else {
...
SIDE.model.data.Metadata.getContentTypes = function(nsp) {
...
SIDE.model.data.Metadata.getDataListItems = function(nsp) {
}
