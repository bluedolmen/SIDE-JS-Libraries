...
SIDE.view.ui.Tree = function(config) {
	...
	this.messageName = config.messageName || '/repository/uuid';		
};
 ...
 * @returns
 */
SIDE.view.ui.Tree.prototype.getDataUrlById = function(path, ticket) {
 ...
 * @returns
 */
SIDE.view.ui.Tree.prototype.getDataUrlByPath = function(path, ticket) {
...
//	var myLoader = new SIDE.view.ui.AlfrescoTreeLoader.getInstance(config);
    ...
    tree.on('click', function (node) {
       	if (node != null) {
       		if (node.leaf) {
       			// It's a file
	           	SIDE.pattern.Observer.publish(instance.messageName, [{"value": node}]);
           	} else {
           		SIDE.pattern.Observer.publish(instance.messageName + '/folder', [{"value": node}]);
           	}
        }
    });
